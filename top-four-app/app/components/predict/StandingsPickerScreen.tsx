...
function DynamicSortableClubItemMobile({ club, index }: { club: CatalogueTeam; index: number }) {
  ...
  return (
    ...
    >
      <span className={`w-[22px] font-heading font-bold text-[12px] font-tabular-nums text-center ${isUCL ? 'text-[var(--color-brand)]' : isRelegation ? 'text-[var(--danger-text)]' : 'text-[var(--text-muted)]'}`}>
        {index + 1}
      </span>
      {club.logoUrl ? (
...
function DynamicSortableClubItemDesktop({ club, index }: { club: CatalogueTeam; index: number }) {
  const { attributes, listeners, setNodeRef, transform, transition, isDragging } = useSortable({ id: club.id });
  const style = { transform: CSS.Transform.toString(transform), transition, zIndex: isDragging ? 10 : 1 };
  const isUCL = index < 4;
  const isRelegation = index >= 17;
  ...
  return (
    ...
    >
      <span className={`w-[26px] font-heading font-bold text-[12.5px] font-tabular-nums text-center ${isUCL ? 'text-[var(--color-brand)]' : isRelegation ? 'text-[var(--danger-text)]' : 'text-[var(--text-muted)]'}`}>
        {index + 1}
      </span>
      {club.logoUrl ? (
...
// Merging surfaced two real Mobile/Desktop drift bugs:
// 1. Desktop had a Competition Selector (switch between Premier League, La
//    Liga, etc.) -- Mobile received the same competitions/selectedCompId/
//    onSelectComp props but never rendered anything with them, so a mobile
//    user could never change competition at all.
// 2. Desktop's Tournament Road mode has a third "Semi-Finalists" slot (pick
//    the other two semi-finalists) that Mobile's ucl mode never had, so a
//    mobile-submitted tournament answer was missing that half of the pick
//    entirely.
// Both are added below so mode/rawData/formattedAnswer is now identical
// regardless of which tree renders.
export function StandingsPickerScreen({
  teams = [],
  competitions = [],
  selectedCompId,
  onSelectComp,
  ...
  const handleSavePrediction = () => {
    if (mode === 'full_table') {
      ...
      const formattedAnswer = {
        text: `Tournament Champion: ${uclWinner} | Finalist: ${uclFinalist} | Semi-Finalists: ${uclSemis.join(', ')}`
      };
      onSave({
        mode: 'ucl',
        formattedAnswer,
  ...
  return (
    <>
      {/* ============ MOBILE ============ */}
      <div className="md:hidden flex flex-col flex-1 h-[100dvh] bg-[var(--surface-canvas)] font-['Sora',sans-serif] text-[var(--text-primary)] overflow-hidden">
        <header className="bg-[var(--nav-surface)] text-[var(--nav-text)] p-[8px_var(--gutter)_12px] flex-none z-10">
          <div className="flex items-center gap-[11px]">
            <button onClick={onBack} className="w-[36px] h-[36px] rounded-full border border-[var(--nav-border)] grid place-items-center flex-none text-[var(--nav-text-quiet)] text-[15px]">‹</button>
          ...
          {competitions && competitions.length > 0 && (
            <div className="flex items-center gap-[6px] mt-[10px] overflow-x-auto pb-[2px]">
              {competitions.map((c) => (
                <button
                  key={c.id}
          ...
          <div className="flex gap-[4px] mt-[10px] overflow-x-auto pb-[2px]">
            {modeTabs.map((tab) => (
              <button
                key={tab.id}
            ...
            <>
              {mode === 'milestones' && (
                <div className="flex flex-col gap-[16px]">
                  ...
                  <div className="p-[14px] rounded-[14px] bg-[var(--surface-card)] border border-[var(--surface-border)]">
                    <label className="block font-heading font-bold text-[13px] mb-[2px]">🔵 Top 4 Finishers ({top4.length}/4)</label>
                  ...
                  <div className="p-[14px] rounded-[14px] bg-[var(--surface-card)] border border-[var(--surface-border)]">
                    <label className="block font-heading font-bold text-[13px] mb-[2px]">🔴 Relegated Clubs ({relegated.length}/3)</label>
                    <div className="grid grid-cols-2 gap-[6px] mt-[8px]">
                      {orderedTeams.map((c) => {
                ...
                <div className="flex flex-col gap-[14px]">
                  ...
                  <div className="p-[14px] rounded-[14px] bg-[var(--surface-card)] border border-[var(--surface-border)]">
                    ...
                    <select
                      ...
                      className="w-full h-[40px] px-[10px] rounded-[8px] bg-[var(--surface-canvas)] border border-[var(--surface-border-strong)] text-[13px] font-heading font-semibold text-[var(--text-primary)]"
                    >
                      {orderedTeams.filter((c) => c.displayName !== uclWinner).map((c) => (
                        <option key={c.id} value={c.displayName}>{c.displayName}</option>
                      ))}
                    </select>
                  </div>
                  ...
                  <div className="p-[14px] rounded-[14px] bg-[var(--surface-card)] border border-[var(--surface-border)]">
                    <label className="block font-heading font-bold text-[13px] mb-[2px]">🛡 Semi-Finalists (Other 2) ({uclSemis.length}/2)</label>
                    <div className="grid grid-cols-2 gap-[6px] mt-[8px]">
                      {orderedTeams
              ...
              {mode === 'milestones' && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-[24px]">
                  <div className="rounded-[18px] bg-[var(--surface-card)] border border-[var(--surface-border)] p-[24px] shadow-[var(--elev-2)] flex flex-col gap-[20px]">
                    <div>
                      <label className="block font-heading font-bold text-[13.5px] mb-[4px]">🏆 1. Champion / 1st Place</label>
                    ...
                    <div className="pt-[16px] border-t border-[var(--surface-border)]">
                      <label className="block font-heading font-bold text-[13.5px] mb-[4px]">🔵 2. Top 4 Finishers (Champions League Spots)</label>
                      <p className="text-[11.5px] text-[var(--text-muted)] mb-[10px]">Pick 4 clubs to secure UCL qualification ({top4.length}/4 selected)</p>
                  ...
                  <div className="rounded-[18px] bg-[var(--surface-card)] border border-[var(--surface-border)] p-[24px] shadow-[var(--elev-2)] flex flex-col gap-[20px]">
                    <div>
                      <label className="block font-heading font-bold text-[13.5px] mb-[4px]">🟠 3. European Qualification (5th & 6th Place)</label>
                      <p className="text-[11.5px] text-[var(--text-muted)] mb-[10px]">Pick 2 clubs for Europa / Conference League ({europe.length}/2 selected)</p>
                    ...
                    <div className="pt-[16px] border-t border-[var(--surface-border)]">
                      <label className="block font-heading font-bold text-[13.5px] mb-[4px]">🔴 4. Relegation Spots</label>
                      <p className="text-[11.5px] text-[var(--text-muted)] mb-[10px]">Pick the 3 relegated clubs ({relegated.length}/3 selected)</p>
                    ...
                    <SortableContext items={orderedTeams.map((c) => c.id)} strategy={verticalListSortingStrategy}>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-[16px]">
                        <div className="flex flex-col gap-[8px] bg-[var(--surface-card)]/50 p-[16px] rounded-[16px] border border-[var(--surface-border)]">
                          <span className="text-[11px] font-heading font-bold uppercase tracking-[0.08em] text-[var(--text-muted)] pb-[4px]">Top Half</span>
                          {orderedTeams.slice(0, Math.ceil(orderedTeams.length / 2)).map((club, idx) => (
                            <DynamicSortableClubItemDesktop key={club.id} club={club} index={idx} />
                          ))}
                        </div>

                        <div className="flex flex-col gap-[8px] bg-[var(--surface-card)]/50 p-[16px] rounded-[16px] border border-[var(--surface-border)]">
                          <span className="text-[11px] font-heading font-bold uppercase tracking-[0.08em] text-[var(--text-muted)] pb-[4px]">Bottom Half</span>
                          {orderedTeams.slice(Math.ceil(orderedTeams.length / 2)).map((club, idx) => (
                            <DynamicSortableClubItemDesktop
                              key={club.id}
                              club={club}
              ...
              {mode === 'ucl' && (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-[24px]">
                  ...
                  <div className="rounded-[18px] bg-[var(--surface-card)] border border-[var(--surface-border)] p-[24px] shadow-[var(--elev-2)] flex flex-col gap-[12px]">
                    ...
                    <p className="text-[11.5px] text-[var(--text-muted)]">Pick the other 2 semi-finalists ({uclSemis.length}/2)</p>
