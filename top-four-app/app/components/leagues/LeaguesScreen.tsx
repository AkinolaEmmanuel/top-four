...
// Merged from LeaguesMobile/LeaguesDesktop. Desktop's own top nav
// (rootNav/user props) was already dead -- DesktopLevelOne in the root
// layout provides the real one, so nothing here ever rendered it. Mobile's
// bottom nav was its own inline copy of the global tab bar rather than the
// shared MobileNav component every other merged screen already uses, and
// that duplication is exactly how it drifted: its Home tab linked to `/`
// instead of `/home` (redirected back to /home by middleware for a signed-in
// visitor, but a wasted hop and inconsistent with every other tab). Now it's
// the same MobileNav everywhere.
