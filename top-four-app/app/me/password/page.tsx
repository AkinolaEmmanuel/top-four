...
export default function PasswordPage() {
  ...
  const handleSave = () => {
    ...
    changePassword.mutate(password.trim(), {
      onSuccess: () => {
        setSuccess(true);
    ...
    <div className="flex-1 bg-[var(--surface-canvas)] flex flex-col h-full text-[var(--text-primary)]">
      <div className="h-[54px] flex items-end px-[24px] pb-[11px] border-b border-[var(--surface-border-strong)] flex-none">
        <div className="flex items-center gap-[15px]">
          ...
          <span className="font-heading font-bold text-[14.5px]">Change Password</span>
        </div>
      </div>
      <div className="p-[24px] max-w-[400px]">
        {success ? (
          <div className="text-[var(--color-success)] text-[14px]">Password successfully changed! Redirecting...</div>
        ) : (
          <>
            <label className="block text-[12px] text-[var(--text-secondary)] mb-[8px]">New Password</label>
            <input 
              type="password"
              value={password}
            ...
            <button 
              onClick={handleSave}
              disabled={!password.trim() || changePassword.isPending}
