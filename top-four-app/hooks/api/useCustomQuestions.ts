...
export function useSubmitCustomAnswer(leagueId: string) {
  ...
  return useMutation({
    mutationFn: ({ questionId, expectedVersion, answer }: { questionId: string, expectedVersion: number, answer: unknown }) => submitCustomAnswer(leagueId, questionId, expectedVersion, answer),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['leagues', leagueId, 'custom-questions'] });
    }
  });
...
export function useResolveCustomQuestion(leagueId: string) {
  ...
  return useMutation({
    mutationFn: ({ questionId, correctAnswer, reason }: { questionId: string, correctAnswer: unknown, reason?: string }) => resolveCustomQuestion(leagueId, questionId, correctAnswer, reason),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['leagues', leagueId, 'custom-questions'] });
    }
  });
